...
namespace Repository.Generics {
    public class RepositoryLong<TEntity> : RepositoryBase where TEntity : class, ISaveableLong<TEntity> {
        public RepositoryLong(DbContext ctx) : base(ctx) { }
        #region Non-Async
        public TEntity GetById(long id, bool includeDefaults = false, params Expression<Func<TEntity, object>>[] includes) {
        ...
        #region Async
        public Task<TEntity> GetByIdAsync(long id, bool includeDefaults = false, params Expression<Func<TEntity, object>>[] includes) {
            var query = GetByIncludes(_entities.Set<TEntity>(), includes, includeDefaults);
        ...
        public Task<bool> EntityExistsAsync(long id) {
