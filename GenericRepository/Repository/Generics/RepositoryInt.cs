...
namespace Repository.Generics {
    public class RepositoryInt<TEntity> : RepositoryBase where TEntity : class, ISaveableInt<TEntity> {
        public RepositoryInt(DbContext ctx) : base(ctx) { }
        #region Non-Async
        public TEntity GetById(int id,bool includeDefaults = false, params Expression<Func<TEntity, object>>[] includes){
        ...
        #region Async
        public Task<TEntity> GetByIdAsync(int id, bool includeDefaults = false, params Expression<Func<TEntity, object>>[] includes){
            var query = GetByIncludes(_entities.Set<TEntity>(), includes, includeDefaults);
